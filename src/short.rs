...
impl Song {
    pub fn new(path: &str, beats: &[f32]) -> Self {
        Self { path: String::from(path), beats: beats.to_vec() }
    }
}

impl Video {
    pub fn new(path: &str, cuts: &[f32]) -> Self {
        Self { path: String::from(path), cuts: cuts.to_vec() }
    }
}
...
fn random_song(options: &[&str]) -> Song {
    let songs: Vec<Song> = vec![
        Song::new("res/audio/metamorphosis.mp3", vec![vec![0., 2.72], (1..35).map(|x| 2.72 + x as f32 * 0.7).collect()].into_iter().flatten().collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/neon-blade.mp3", vec![vec![0., 2.68], (1..18).map(|x| 2.68 + x as f32 * 0.635).collect()].into_iter().flatten().collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/dancin.mp3", vec![vec![0., 3.55], (1..35).map(|x| 3.55 + x as f32 * 0.53).collect()].into_iter().flatten().collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/mtg.mp3", (0..9).map(|x| x as f32 * 1.89).collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/murder-in-my-mind.mp3", (0..35).map(|x| x as f32 * 0.4999999).collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/immaculate.mp3", vec![(0..9).map(|x| x as f32 * 1.).collect::<Vec<f32>>(), (1..18).map(|x| 8. + x as f32 * 0.54).collect()].into_iter().flatten().collect::<Vec<f32>>().as_slice()),
        Song::new("res/audio/miss-you.mp3", vec![vec![0., 3.95], (1..35).map(|x| 3.95 + x as f32 * 0.433333).collect()].into_iter().flatten().collect::<Vec<f32>>().as_slice())
    ];

    loop {
        let song: Song = songs[rand::thread_rng().gen_range(0..songs.len())].clone();

        if options.is_empty() || options.contains(&song.path.as_str()) {
            return song;
        }
    }
}

fn produce_edit(cfg: &serde_json::Value) -> String {
    ...
    create("no-audio.mp4", &song.beats, Config::Edit {
...
fn produce_compare(cfg: &serde_json::Value) -> String {
